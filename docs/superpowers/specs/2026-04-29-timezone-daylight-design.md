...
- **Angular Material** — `mat-select`, `mat-form-field`, time picker (нейтральная база, поверх — кастомные стили)
- **ng2-charts + Chart.js** — линейный график
- **@angular/localize** — i18n-инфраструктура (RU only в MVP)
- **SCSS** — кастомизация под визуальное направление (см. 6.5)
- **SVG-иллюстрации** для hero-блока (готовые из библиотеки undraw.co / Storyset, либо кастом — см. 6.5)
...
└── src/app/
    ...
    │   ├── hero-illustration/                 ← парные SVG: «Утро» и «Вечер» (см. 6.5)
    │   ├── region-selector/
    ...
    ├── shared/
    └── assets/illustrations/                  ← SVG-иллюстрации (морнинг, ивнинг)
```
...
│  HERO                                                         │
│  ┌──────────────┐                       ┌──────────────┐     │
│  │  УТРО (SVG)  │   «Россия живёт      │ ВЕЧЕР (SVG)  │     │
│  │  ☀️ светит,  │   в неудобном        │ 🌙 темно,    │     │
│  │  😴 спит      │    времени»          │ 📺 у ТВ      │     │
│  └──────────────┘                       └──────────────┘     │
│  Подзаголовок: «Утром солнце уже встало, а вы ещё спите.     │
│                Вечером ещё лето, но за окном уже ночь.»       │
├──────────────────────────────────────────────────────────────┤
...
На мобильных иллюстрации перестраиваются вертикально (утро сверху, вечер снизу), подзаголовок остаётся между ними или ниже.
...
### 6.5 Визуальное направление

Визуальный дизайн — это не вторичный аспект, а ключевой инструмент донесения тезиса. Картинки должны работать так же сильно, как цифры.

**Hero-иллюстрации (две парные SVG):**

1. **«Утро»** — за окном спальни уже встало яркое солнце, лучи падают на кровать; в кровати спит человек. Подпись (рядом или под картинкой): _«Солнце уже взошло — а вы ещё спите»_.
2. **«Вечер»** — лето, 19:00, небо тёмное / ночное; в комнате семья (взрослый + дети) сидит у мерцающего телевизора. За окном — темнота. Подпись: _«Ещё лето, ещё рано — а на улице уже ночь»_.

Иллюстрации должны выглядеть в едином стиле (одна и та же иллюстраторская рука). Допустимые источники:
- **Готовые наборы** — `unDraw` (CC0), `Storyset` (free with attribution). Подбираем близкие по стилю и слегка адаптируем цвета под палитру сайта.
- **Кастомная отрисовка** — если найти подходящие готовые не получится; в этом случае — flat illustration с тёплыми/холодными акцентами.

В коде хранятся как `assets/illustrations/morning.svg` и `assets/illustrations/evening.svg`. Lazy-load не требуется (общий вес < 100 КБ).

**Цветовая палитра:**
- Основные: тёплый янтарь / охра (солнце, утро) и глубокий синий / индиго (ночь, вечер).
- Нейтральные: off-white фон, графитовый текст.
- Акцент в карточках «Разница» — янтарный (положительная динамика).

**Типографика:**
- Заголовки — крупный жирный sans-serif (например, `Manrope`, `Inter`, `IBM Plex Sans`).
- Тело — тот же sans-serif, regular.
- Цифры в карточках — особо крупно (48–64 px), tabular-nums для выравнивания.

**Микро-анимации (без перегруза):**
- Появление карточек: fade + slide-up (200–300 мс).
- Изменение чисел: count-up при пересчёте.
- Иллюстрации — статичные, без сложных анимаций (Lottie не используем для MVP).

**Адаптивность:**
- Брейкпоинты: ≥1024 (desktop), 640–1023 (tablet), <640 (mobile).
- Hero на мобильных — иллюстрации друг под другом с подзаголовком между.
- Карточки на мобильных — вертикальный стек.
- График — горизонтальный скролл при ширине экрана <640.
...
Текущая спека описывает **этап 1** — самодостаточный калькулятор без аутентификации и БД. Социальный слой (голосование, комментарии) выносится в **этап 2** — отдельная спека (`2026-XX-XX-vk-voting-and-comments-design.md`), которую напишем после релиза этапа 1.

Отложено на этап 2 (или позже):
- **VK OAuth** — регистрация и вход через `vk.com`.
- **Голосование** «за / против перевода часов в Кировской области» — БД, антифрод, отображение итогов.
- **Комментарии** — обсуждение под результатами, модерация, антиспам.

Отложено в неопределённое будущее (вне обоих этапов):
...
- Сохранение пользовательских предпочтений калькулятора (региона, времени).
